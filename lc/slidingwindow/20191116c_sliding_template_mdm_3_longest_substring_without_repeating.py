...
class Solution():
    ...
    """
    """
    塊の中から条件Xをみたす連続値Tを探せ、という問題のテンプレが使えそうな問題。
    問題は、文字列の中から重複しない文字列を探せ
    という問題。最終的に最大文字列数を返すので逐次、maxを更新する。
    
    塊(文字列S)の中から条件X(重複しない)文字列Tを探せ。
    とう問です。
    
    条件Xが重複しない文字列tなのえhampを持って重複を検知し、
    違反すればcounterをインクリしよう。
...
samples = [
    ...
    ("bbbbb", 1),
    ("pwwkew", 3),
    ("oongktizbxdfllkhoatmmjejwvxgieayspfeipfkmjrzwbhmdazpjfahczokdqlxfqcnfhxrsoqqsz", 10),
]
