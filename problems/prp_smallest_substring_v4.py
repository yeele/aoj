...
def get_shortest_unique_substring(arr, S):
    ...
    while i < len(S):
            ...
            if S[i] != watch:
                if watch in matched:
                    ...
                    if watch not in S[i:j+1]:
                        ...
                        break
                    else: ans = update_ans(S, arr, matched, i, j, ans)
                else: ans = update_ans(S, arr, matched, i, j, ans)
            else:
                ans = update_ans(S, arr, matched, i, j, ans)
        ...
        if j < len(S)-1:
            ...
            j+=1
        print("*"*20)
        if i >= len(S)-1 and j >= len(S)-1: break
...
#print(get_shortest_unique_substring(['a', 'b'], "bzzzzaxyxybc"))
#print(get_shortest_unique_substring(['a', 'b'], "bzzzzaxyxyabchhh"))
